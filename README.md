...
This repository contains the code, experiments, and conference poster for evaluating unsupervised clustering techniques on raw human pose data extracted from video streams. The study compares K-Means, DBSCAN, and Agglomerative Clustering for discovering discrete body postures without manual annotations.
...
## Technical Stack
- **Data Extraction:** MediaPipe Landmarker (33 3D landmarks)
- **Preprocessing:** StandardScaler, Dual-Stage Heuristic Filtering
- **Algorithms:** K-Means (Kneedle/Elbow optimization), DBSCAN (with OPTICS), Agglomerative Hierarchical Clustering
- **Validation:** t-SNE, PCA, Silhouette Coefficient

## Key Findings
- Successfully reduced a noisy dataset of 13,474 frames to 4,789 high-fidelity frames.
- DBSCAN proved superior at isolating "transitional" movements as noise, maintaining high cluster purity for discrete postures.
- Demonstrated that raw coordinate streams contain high discriminative power without the need for complex latent feature extraction.

## Repository Structure

├── Kmeans/            # K-Means clustering implementation
├── DBSCANCode/        # DBSCAN + OPTICS implementation
├── AgglomCode/        # Agglomerative clustering implementation
├── 3dPlottingCode/    # PCA, t-SNE and visualisation scripts
├── Data/              # Processed pose coordinate datasets
├── Pose Grouping Poster.pdf
└── Pose Grouping Poster.jpg

## Poster Preview

![Research Poster](Poster.jpg)

*Presented at the SPECS Research Conference 2026, University of Hertfordshire.*
...
> **Automating data annotation for computer vision models by clustering raw 3D human pose coordinates.** 
> Presented at the School of Physics, Engineering and Computer Science (SPECS) Conference, University of Hertfordshire.
...
* `3dPlottingCode/` - Visualization scripts used to generate the 3D stick-figure pose grids and t-SNE/PCA scatter plots.
...
1. **K-Means (Baseline):** 
   * Used the Kneedle algorithm to identify an inflection point at `k=6` discrete poses.
2. **Agglomerative Clustering:** 
   * Validated our `k=6` choice by generating a Dendrogram (distance threshold = 175), showing stable cluster branches between 5 and 7.
3. **DBSCAN (with OPTICS):** 
   * **Key Finding:** DBSCAN was exceptionally effective at isolating the awkward, transitional frames between dance moves and identifying them as **"noise"** (Cluster -1). This kept the primary physical posture clusters incredibly pure compared to K-Means.

*(Note: t-SNE and PCA visualizations confirmed clear spatial separation of the resulting clusters).*
